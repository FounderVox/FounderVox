...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  const analyzeAudio = () => {
    ...
    let sum = 0;
    for (let i = 0; i < externalDataArray.length; i++) {
      const value = externalDataArray[i] / 255;
      sum += value * value;
    }
    const rms = Math.sqrt(sum / externalDataArray.length);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const update = (t: number) => {
        ...
        if (enableVoiceControl && externalAnalyser && externalDataArray) {
